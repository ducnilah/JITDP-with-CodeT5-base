...
import json
from transformers import AutoTokenizer, RobertaTokenizer, RobertaModel, RobertaConfig, Trainer, TrainingArguments
import torch
...
class VulBERTaWithNumeric(nn.Module):
    def __init__(self, model_name="claudios/VulBERTa-MLP-ReVeal", num_labels=2, num_numeric=13, class_weights=None):
        super().__init__()
        ...
        self.classifier = nn.Sequential(
            nn.Linear(self.config.hidden_size + 64, 512),  # Larger first layer
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(512, 256),
            nn.ReLU(),
...
path_1 = '/kaggle/input/jitdp-xxx/updated_defects.jsonl'
path_2 = '/kaggle/input/jitdp-xxx/updated_non_defects.jsonl'
path_3 = '/kaggle/input/jitdp-xxx/updated_fixes.jsonl'
...
path_4 = '/kaggle/working/labels.jsonl'
path_5 = '/kaggle/input/jitdp-xxx/patches-FFmpeg.jsonl'
path_6 = '/kaggle/input/jitdp-xxx/features-FFmpeg.jsonl'
...
# Skipped indicies here are samples that having utf pretrained models could not handle, I searched all these indicies in a different file code.
...
model = VulBERTaWithNumeric('claudios/VulBERTa-MLP-ReVeal', num_labels=2, num_numeric=13, class_weights=class_weights)
tokenizer = AutoTokenizer.from_pretrained('claudios/VulBERTa-MLP-ReVeal', trust_remote_code=True)
...
wandb.login(key='')
wandb.init(project="jitdp-vulberta", name='experiment-1')
