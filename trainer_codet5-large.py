...
class CodeT5WithNumeric(nn.Module):
    def __init__(self, model_name="Salesforce/codet5-large", num_labels=2, num_numeric=13, class_weights=None):
        super().__init__()
...
model = CodeT5WithNumeric('Salesforce/codet5-large', num_labels=2, num_numeric=13, class_weights=class_weights)
tokenizer = AutoTokenizer.from_pretrained('Salesforce/codet5-large', use_fast=False)
...
def compute_metrics(eval_pred, threshold=0.42):
...
wandb.login(key='')
wandb.init(project="jitdp-codet5-large", name='experiment-2')
